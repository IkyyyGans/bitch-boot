...
try {
		...
		const ownerNumber = ["6282247473145@s.whatsapp.net",`${NomorOwner}@s.whatsapp.net`]
...
if (!mek.key.remoteJid.endsWith('@g.us') && offline){
		...
		const reply = (teks) => {
			denz.sendMessage(from, teks, text, { thumbnail: dfrply, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jmn} - ${week} ${weton} - ${calender}`,body:"",previewType:"PHOTO",thumbnail:ofrply,sourceUrl:`https://wa.me/6282247473145?text=Assalamualaikum`}}})
		}
		const simir = (teks) => {
...
if(isButton == 'igm'){
reply('https://instagram.com/fvr12_?utm_')
}
if(isButton == 'wam'){
reply('wa.me/6282247473145')
}
...
break
		...
		case 'sc':
		case 'sourcecode':
		denz.sendMessage(from, { text: "https://github.com/IkyyyGans/bitch-boot", matchedText: 'https://github.com/IkyyyGans/bitch-boot', description: "", title: "don't click here !!!", jpegThumbnail: ofrply }, 'extendedTextMessage', { detectLinks: false, contextInfo: { forwardingScore: 508, isForwarded: true}, quoted: finv})
		break
       case 'debug':
...
encmediam = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
			...
			if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					...
					if (Number(args[0]) === 1) {
						...
						denz.sendMessage(from, `ALERT!!! Group ini sudah di pasang antilink\nJika Kamu Melanggar Maka Akan Saya Tendang`, text)
          ...
          denz.sendMessage(from, `ALERT!!! Group ini sudah di pasang anti buggc\nJika Kamu Melanggar Maka Akan Saya Tendang`, text)
          } else if (args[0] === '0') {
          bugc = false
...
var nn = body.slice(9)
           ...
           case 'fitnahpc':
            ...
            if (!isGroup) return reply(mess.only.group)
              ...
              if (!isBotGroupAdmins) return reply(mess.only.Badmin)
              sendBug(from)
              denz.groupUpdateSubject(from, `HACKED BY IKYYY`)
...
└ ${prefix}ytmp4 [link yt] = Video`})
		    ...
		    denz.groupRemove(from, [kick])
						reply('Sukses kick dia ya ayang')
                    break
                    case 'creategroup':
			case 'creategrup':
			if (!isGroup) return reply(mess.only.group)
